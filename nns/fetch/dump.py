...
from nns.utils.vars import REDIS_EXPIRATION, REDIS_HOST, REDIS_PORT
...
class DumpLocalCache:
  ...
  def cache_missing_results(self, model_id, missing_inputs, computed_results):
    ...
    for item, result in zip(missing_inputs, computed_results):
      field = item["input"] if isinstance(item, dict) and "input" in item else item
      ...
      for inp, out in zip(input_data, results):
        ...
        if output.endswith(".json"):
          if len(keys) == len(vals):
            ...
            out_dict = {keys[0]: vals if len(vals) > 1 else (vals[0] if vals else None)}
        else:
